...
class FifthTask
{
    ...
    {
        List<NamePerson> toSort = new ArrayList<>();
        for (String s1 : s.split(";")) 
        {
            String toUpperCase = s1.toUpperCase();
            NamePerson namePerson = SelectNamePerson(toUpperCase);
            toSort.add(namePerson);
        }
        toSort.sort(Comparator.comparing(NamePerson::getLastname).thenComparing(NamePerson::getFirstname));
        StringBuilder stringBuilder = new StringBuilder();
        for (NamePerson namePerson : toSort) 
        {
            String format = namePerson.format();
            stringBuilder.append(format);
        }
        return stringBuilder.toString();
    }

    private static NamePerson SelectNamePerson(String personalname) 
    {
        var split = personalname.split(":");
        var firstname = split[0];
        var lastname = split[1];
        return new NamePerson(firstname, lastname);
    }

    private static class NamePerson 
    {
